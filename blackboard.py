...
class BlackBoardClient:
    ...
    class LearnAPIVersion:
        """
        Represents a Blackboard Learn API Version for comparison
        """

        def __init__(self, learn_version: str):
            """
            Parse Blackboard Learn API version with enhanced format support
            Handles formats like: '3900.0.0', '3900.0.0-rel.40+6f84295'

            :param learn_version: Version string from Blackboard API
            """
            import re

            # Extract only the numeric version components
            version_match = re.match(r'^(\d+)\.(\d+)\.(\d+)', str(learn_version))
            ...
            if version_match:
                ...
                parts = re.findall(r'\d+', str(learn_version))
                self.major = int(parts[0]) if len(parts) > 0 else 0
                self.minor = int(parts[1]) if len(parts) > 1 else 0
                self.patch = int(parts[2]) if len(parts) > 2 else 0

            self._raw = [str(self.major), str(self.minor), str(self.patch)]
        ...
        def __repr__(self):
            return f"LearnAPIVersion({str(self)})"
    ...
    def send_get_request(self, endpoint: str, silent_on_error=False, **kwargs) -> Optional[requests.Response]:
        """
        ...
        :param silent_on_error: If True, don't print REST exceptions (useful for optional API calls)
...
class BlackBoardContent:
    ...
    def attachments(self, existing_list=None, endpoint=None) -> List[BlackBoardAttachment]:
        ...
        # REMOVED: Don't skip content types - check ALL content for attachments
        # The old code was skipping documents, but PDFs are often embedded in documents!

        if endpoint is None:
            endpoint = BlackBoardEndPoints.get_file_attachments(self.course.id, self.id)

        try:
            request = self.client.send_get_request(endpoint, silent_on_error=True)
            if request is None:
                return existing_list
            ...
            # Silently skip request errors (many documents return 400)
            pass
    ...
    """
    Add this method to the BlackBoardContent class in blackboard.py
    Place it right after the attachments() method (around line 850)
    """

    def extract_pdf_links_from_body(self) -> List[Dict[str, str]]:
        """
        Extract PDF download links embedded in the HTML body content
        Returns list of dicts with 'url' and 'filename'
        """
        import re
        ...
        # Method 1: Look for data-bbfile attributes (Ultra format)
        bbfile_pattern = r'data-bbfile="({[^"]+})"'
        bbfile_matches = re.findall(bbfile_pattern, str(self.body))
    ...
    def download_embedded_pdfs(self, location: str) -> int:
        """
        Download PDFs that are embedded in the HTML body
        Returns the number of PDFs downloaded
        """
        pdf_links = self.extract_pdf_links_from_body()
        downloaded = 0
        ...
        for pdf_info in pdf_links:
            try:
                ...
                download_location = os.path.abspath(os.path.join(location, filename_safe))
                download_directory = os.path.dirname(download_location)

                # Download the PDF
                response = self.client.session.get(url, allow_redirects=True)
                ...
                if response.status_code == 200:
                    ...
                    if os.path.isfile(download_location):
                        ...
                        _println(f"{Fore.YELLOW}[UP TO DATE] {filename_safe}\n[LOCATION] {download_directory}")
                    else:
                        with open(download_location, 'wb') as f:
                            f.write(response.content)
                        from colorama import Fore
                        _println(f"{Fore.GREEN}[DOWNLOADED] {filename_safe}\n[LOCATION] {download_directory}")
                        downloaded += 1
                else:
                    from colorama import Fore
                    _println(f"{Fore.RED}[FAILED] {filename_safe} - Status: {response.status_code}")
            except Exception as e:
                from colorama import Fore
                _println(f"{Fore.RED}[ERROR] {pdf_info.get('filename', 'unknown')}: {str(e)}")
...
def get_cookies(learn_url: str, default_browser: Optional[str] = None) -> CookieJar:
    ...
    def get_cookies_from_browser(browser_name: str, browser_fn: Callable[..., CookieJar]):
        try:
            _println("Attempting to Grab Cookies for \"{}\" from browser: {}", learn_domain, browser_name,
                     new_line=False)
